...
async function testCalculateFees() {
  try {
    ...
    for (const product of products) {
      ...
      productPrices[product.id] = variant?.price || 0;
    }
    
    const sampleCart = [
      { productId: products[0]?.id, price: productPrices[products[0]?.id] || 500, quantity: 1, templeId: products[0]?.templeId },
      { productId: products[1]?.id, price: productPrices[products[1]?.id] || 300, quantity: 2, templeId: products[1]?.templeId }
    ];
    ...
    if (totalPlatformFee === 0) {
      ...
      console.log('This confirms frontend issue!');
    } else {
