...
import { Bell, CheckCheck, Clock, ShieldAlert, Package, Search } from "lucide-react";
...
export default function SellerNotificationsPage() {
    ...
    useEffect(() => {
        ...
        if (userStr) {
            setSellerUser(JSON.parse(userStr));
        }
    }, []);

    const userId = sellerUser?.id || sellerUser?.email;
    ...
    return (
        <div className="space-y-6 max-w-5xl mx-auto">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h1 className="text-2xl font-bold tracking-tight">Seller Notifications</h1>
                    <p className="text-muted-foreground">
                        Stay updated on your store activity, orders, and system alerts.
                    </p>
                </div>
                {unreadCount > 0 && (
                            ...
                            >
                                ...
                                <div className="flex-1 min-w-0 space-y-1.5 pt-1">
                                    <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-2">
                                        <h4 className={cn(
                                            "text-base leading-none pr-8 font-bold",
                                            !n.isRead ? "text-foreground" : "text-foreground/80 font-semibold"
                                        )}>
                                            {n.title}
                                        </h4>
