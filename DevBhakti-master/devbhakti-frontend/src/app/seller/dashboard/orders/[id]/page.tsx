...
import { fetchSellerOrders, updateSellerSubOrderStatus } from "@/api/sellerController";
...
export default function SellerOrderDetailsPage() {
    ...
    const handleStatusUpdate = async (status: string) => {
        try {
            const response = await updateSellerSubOrderStatus(params.id as string, { status });
            if (response.success) {
    ...
    if (loading) {
        return (
            <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4">
                ...
                <p className="text-[#794A05] font-medium">Loading Order Details...</p>
            </div>
        );
    }
    ...
    return (
        <div className="space-y-6 max-w-6xl mx-auto pb-12">
            <div className="flex items-center gap-4">
                ...
                <div>
                    <h1 className="text-2xl font-bold font-serif">Order Details</h1>
                        ...
                        <CardContent className="p-0">
                                ...
                                <TableBody>
                                    {order.items.map((item: any) => (
                                        <TableRow key={item.id}>
                                            ...
                                            <TableCell className="text-right pr-6 font-bold text-[#794A05]">₹{item.price.toLocaleString()}</TableCell>
                                        </TableRow>
                                    ))}
                                </TableBody>
                            </Table>
                            <div className="p-6 bg-slate-50/50 border-t space-y-2">
                                ...
                                <div className="flex justify-between text-lg font-bold border-t pt-2">
                                    ...
                                    <span className="text-[#794A05]">₹{order.totalAmount.toLocaleString()}</span>
                                </div>
                            </div>
                        </CardContent>
                    </Card>
                        ...
                        <CardContent>
                            <div className="flex flex-wrap gap-4">
                                {canManageOrders ? (
                                    ...
                                    <p className="text-sm text-muted-foreground italic">You don't have permission to update status.</p>
                                )}
                                <Button 
                                    variant="outline" 
                                    className="h-12 rounded-xl flex items-center gap-2"
                                    onClick={() => router.push(`/seller/dashboard/orders/print?ids=${order.id}`)}
                                >
                        ...
                        <CardContent className="pt-6 space-y-4">
                            ...
                            <div>
                                ...
                                <p className="font-bold text-slate-900 flex items-center gap-2">
                                    <Phone className="w-3 h-3 text-[#794A05]" /> {order.order?.user?.phone || "N/A"}
                                </p>
                            </div>
                        </CardContent>
                    </Card>
                        ...
                        <CardContent className="pt-6">
                            <div className="text-sm space-y-1">
                                ...
                                <p className="text-slate-600 leading-relaxed">
                                    ...
                                    <span className="font-bold text-[#794A05]">{order.order?.shippingAddress?.pincode}</span>
                                </p>
                            </div>
                        </CardContent>
                    </Card>
