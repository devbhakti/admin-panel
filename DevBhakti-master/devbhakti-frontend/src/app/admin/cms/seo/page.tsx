...
export default function SeoCmsPage() {
  ...
  const currentPageData = settings[activePage]?.[activeTab] || { title: "", description: "", keywords: "" };
              ...
              <div className="pt-6 border-t border-slate-100">
                ...
                <div className="bg-white border border-slate-200 rounded-xl p-6 max-w-2xl shadow-sm">
                  <div className="text-xs text-slate-500 mb-1 flex items-center gap-1 truncate">
                    https://devbhakti.in {activeTab !== 'en' && `/${activeTab}`} {currentPageInfo?.path !== "/" && <span className="flex items-center"><ChevronRight className="w-3 h-3" /> {currentPageInfo?.path.substring(1)}</span>}
                  </div>
                  <div className="text-xl text-[#1a0dab] hover:underline cursor-pointer mb-2 font-medium truncate">
                    {currentPageData.title || (activeTab === 'hi' ? 'पृष्ठ का शीर्षक' : activeTab === 'mr' ? 'पृष्ठाचे शीर्षक' : 'Page Title Preview')}
                  </div>
                  <div className="text-sm text-[#4d5156] leading-relaxed line-clamp-2">
                    {currentPageData.description || (activeTab === 'hi' ? 'खोज परिणामों में आपके पृष्ठ के दिखाई देने का संक्षिप्त विवरण यहां दर्ज करें।' : activeTab === 'mr' ? 'शोध इंजिन निकालांमध्ये तुमचे पृष्ठ कसे दिसेल याचा संक्षिप्त सारांश येथे प्रविष्ट करा.' : 'Enter a meta description to see how your page will appear in search engine results.')}
                  </div>
                </div>
              </div>
            </CardContent>
