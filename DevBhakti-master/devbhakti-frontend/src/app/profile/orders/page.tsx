            ...
            <main className="pt-24 pb-16 container mx-auto px-4 relative">
                ...
                <div className="max-w-4xl mx-auto relative z-10">
                    <div className="flex items-center gap-3 mb-6">
                        ...
                        <div>
                            <h1 className="text-2xl font-serif font-bold text-slate-900 tracking-tight">{t("orders.page_title")}</h1>
                            <p className="text-[13px] text-slate-500 font-medium">{t("orders.page_subtitle")}</p>
                        </div>
                    </div>
                    ...
                    {isLoading ? (
                        ...
                        <Card className="rounded-[2.5rem] border-dashed border-2 p-12 text-center bg-white/50">
                            ...
                            <h3 className="text-xl font-bold text-slate-800 mb-2">{t("orders.empty_title")}</h3>
                            <p className="text-slate-500 mb-6">{t("orders.empty_subtitle")}</p>
                                    ...
                                    <div className="p-6 md:p-8">
                                        <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">{t("orders.label_order_id")}</p>
                                                    <p className="font-mono text-sm font-bold text-slate-900">#{order.id.slice(-8).toUpperCase()}</p>
                                                </div>
                                            </div>
                                            <div className="flex flex-col items-end">
                                                <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-1">{t("orders.label_status")}</p>
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 py-6 border-y border-slate-50">
                                            <div className="flex items-start gap-5 col-span-1 md:col-span-1 lg:col-span-1">
                                                ...
                                                <div>
                                                    <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-1.5">{t("orders.label_products")}</p>
                                                    <p className="text-sm font-black text-slate-900 line-clamp-1">
                                                        {order.subOrders.flatMap((so: any) => so.items)[0]?.product?.name}
                                                        {order.subOrders.flatMap((so: any) => so.items).length > 1 ? ` ${t("orders.and_more")}` : ""}
                                                    </p>
                                                    <p className="text-[11px] text-[#794A05] font-bold mt-0.5">{order.subOrders.reduce((acc: number, so: any) => acc + so.items.length, 0)} {t("orders.items_count")}</p>
                                                </div>
                                            </div>
                                            <div>
                                                <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-2.5">{t("orders.label_shipping_to")}</p>
                                                <div className="flex items-start gap-2.5 bg-slate-50/50 p-2.5 rounded-2xl border border-slate-100/50">
                                                    ...
                                                    <div>
                                                        <p className="text-sm font-bold text-slate-800 leading-tight">{order.shippingAddress?.fullName || t("orders.not_provided")}</p>
                                                        <p className="text-[10px] text-slate-500 font-medium mt-0.5">{order.shippingAddress?.city}, {order.shippingAddress?.pincode}</p>
                                                    </div>
                                                </div>
                                            </div>
                                            <div className="lg:text-right">
                                                <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-3">{t("orders.label_order_value")}</p>
                                                <p className="text-3xl font-black text-[#794A05] tracking-tighter drop-shadow-sm">₹{order.totalAmount?.toLocaleString()}</p>
                                                <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-1">{t("orders.inclusive_taxes")}</p>
                                            </div>
                                        </div>
                                        ...
                                        <div className="mt-6 flex items-center justify-between">
                                            ...
                                            <div className="flex items-center gap-2">
                                                ...
                                                >
                                                    {expandedOrderId === order.id ? t("orders.btn_hide_details") : t("orders.btn_view_details")}
                                            ...
                                            {expandedOrderId === order.id && (
                                                ...
                                                >
                                                    <div className="pt-8 space-y-8">
                                                        <div className="grid md:grid-cols-2 gap-6">
                                                            <div className="bg-slate-50/50 p-6 rounded-3xl border border-slate-100 flex flex-col gap-4">
                                                                <h4 className="text-xs font-bold text-slate-400 uppercase tracking-widest flex items-center gap-2">
                                                                    ...
                                                                    {t("orders.section_shipping")}
                                                                </h4>
                                                                <div className="space-y-3">
                                                                    <div>
                                                                        <p className="text-[10px] text-slate-400 font-bold uppercase tracking-tighter">{t("orders.recipient_name")}</p>
                                                                        <p className="text-sm font-bold text-slate-700">{order.shippingAddress?.fullName}</p>
                                                                    </div>
                                                                    <div>
                                                                        <p className="text-[10px] text-slate-400 font-bold uppercase tracking-tighter">{t("orders.address")}</p>
                                                            ...
                                                            <div className="bg-orange-50/30 p-6 rounded-3xl border border-orange-100/50 flex flex-col gap-4">
                                                                <h4 className="text-xs font-bold text-slate-400 uppercase tracking-widest flex items-center gap-2">
                                                                    ...
                                                                    {t("orders.section_items_info")}
                                                                </h4>
                                                                <div className="space-y-3">
                                                                    <div className="flex justify-between items-center">
                                                                        <p className="text-[10px] text-slate-400 font-bold uppercase tracking-tighter">{t("orders.total_items")}</p>
                                                                        <p className="text-sm font-bold text-slate-700">{order.subOrders.reduce((acc: number, so: any) => acc + so.items.length, 0)} {t("orders.units")}</p>
                                                                    </div>
                                                                    <div className="flex justify-between items-center">
                                                        ...
                                                        {order.subOrders.some((so: any) => so.awbCode) && (
                                                            <div className="p-6 bg-blue-50/50 rounded-3xl border border-blue-100 flex items-center justify-between">
                                                                <div className="flex items-center gap-4">
                                                                    ...
                                                                    <div>
                                                                        <p className="text-[10px] text-blue-500 font-bold uppercase tracking-widest">{t("orders.tracking_label")}</p>
                                                                        <p className="text-sm font-bold text-slate-900">{t("orders.tracking_synced")}</p>
                                                                    </div>
                                                                </div>
                                                                <div className="flex gap-3">
                                                                    {order.subOrders.map((sub: any) => sub.awbCode && (
                                                                        ...
                                                                        >
                                                                            {t("orders.track_pkg")}{sub.id.slice(-4)}
                                                                        </Button>
                                                                    ))}
                                                                </div>
                                                            </div>
                                                        ...
                                                        <div className="space-y-4">
                                                            <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] pl-1">{t("orders.package_breakdown")}</h4>
                                                                ...
                                                                <div key={sub.id} className="bg-white border border-slate-100 rounded-[2rem] overflow-hidden shadow-sm hover:shadow-md transition-shadow">
                                                                    <div className="bg-slate-50/40 px-6 py-3 border-b border-slate-50 flex items-center justify-between">
                                                                        <span className="text-[11px] font-bold text-slate-500 flex items-center gap-1.5">
                                                                            ...
                                                                            {sub.temple?.name || t("orders.official_warehouse")}
                                                                        </span>
                                                                                    ...
                                                                                    <div>
                                                                                        ...
                                                                                        <div className="flex items-center gap-2 mt-0.5">
                                                                                            <span className="text-[10px] text-slate-400 font-black uppercase tracking-tighter">
                                                                                                {item.variantName || t("orders.standard_edition")}
                                                                                            </span>
                                                                                            <span className="w-1 h-1 rounded-full bg-slate-200" />
                                                                                            <span className="text-[10px] text-orange-600 font-bold">{item.quantity} {t("orders.unit_s")}</span>
                                                                                        </div>
                                                                                    </div>
                                                                                </div>
                                                                                <div className="text-right">
                                                                                    <p className="text-sm font-black text-slate-900">₹{(item.price * item.quantity).toLocaleString()}</p>
                                                                                    <p className="text-[10px] text-slate-400 font-medium">₹{item.price} {t("orders.each")}</p>
                                                                                </div>
                                                                            </div>
                                                                        ))}
                                                                    </div>
                                                        ...
                                                        <div className="bg-orange-50/30 p-6 rounded-[2rem] border border-orange-100/50 space-y-3 mb-4">
                                                            <div className="flex justify-between items-center text-sm">
                                                                <span className="font-bold text-slate-500 uppercase tracking-widest text-[10px]">{t("orders.items_subtotal")}</span>
                                                            ...
                                                            <div className="flex justify-between items-center text-sm pt-2 border-t border-orange-200/30">
                                                                <span className="font-bold text-slate-500 uppercase tracking-widest text-[10px]">{t("orders.platform_fee")}</span>
                                                                <span className="font-black text-slate-700">₹{(order.platformFee || 0).toLocaleString()}</span>
                                                            </div>
                                                            {(order.shippingCost || 0) > 0 && (
                                                               <div className="flex justify-between items-center text-sm pt-2 border-t border-orange-200/30">
                                                                   <span className="font-bold text-slate-500 uppercase tracking-widest text-[10px]">{t("orders.shipping_cost")}</span>
                                                        ...
                                                        <div className="flex items-center justify-between p-6 bg-slate-900 rounded-[2.5rem] text-white shadow-xl shadow-slate-900/10">
                                                            <div className="flex items-center gap-4">
                                                                ...
                                                                <p className="font-serif font-bold text-lg flex items-center gap-2">
                                                                    ...
                                                                    {order.status === 'DELIVERED' ? t("orders.status_delivered") : t("orders.status_processing")}
                                                                </p>
                                                            </div>
                                                            <div className="text-right hidden sm:block">
                                                                <p className="text-[10px] text-white/50 font-bold uppercase tracking-widest mb-1">{t("orders.final_payment")}</p>
