      ...
      <SheetContent className="w-full sm:max-w-lg flex flex-col bg-[#fdf6e9] border-[#794A05]/10">
        <SheetHeader>
          <SheetTitle className="flex items-center gap-2 font-display text-[#2a1b01]">
            ...
            {t('marketplace.cart.title')} ({items.length} {t('marketplace.cart.items_label')})
          </SheetTitle>
        </SheetHeader>
        ...
        <div className="flex-1 overflow-y-auto py-4 scrollbar-thin scrollbar-thumb-[#794A05]/20">
          {items.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-center p-8">
              ...
              <p className="text-xl font-display font-bold text-[#2a1b01] mb-2">{t('marketplace.cart.empty_title')}</p>
              <p className="text-slate-500 mb-8">{t('marketplace.cart.empty_subtitle')}</p>
              <Button
                variant="outline"
              ...
              >
                {t('marketplace.cart.start_exploring')}
              </Button>
            </div>
          ) : (
          ...
          <SheetFooter className="flex-col gap-4 sm:flex-col p-6 bg-white border-t border-[#794A05]/10 rounded-t-3xl shadow-[0_-8px_30px_rgb(0,0,0,0.04)]">
            <div className="w-full space-y-3">
              <div className="flex justify-between text-sm">
                <span className="text-slate-500">{t('marketplace.cart.subtotal')}</span>
              ...
              <div className="flex justify-between text-sm">
                <span className="text-slate-500">{t('marketplace.cart.platform_fee')}</span>
                <span className="flex items-center font-bold text-[#794A05]">
                  {isCalculating ? (
                    <span className="text-[10px] animate-pulse">{t('marketplace.cart.calculating')}</span>
                  ) : (
              ...
              <div className="flex justify-between text-xs text-slate-400 italic">
                ...
                <span>{t('marketplace.cart.calculated_at_checkout')}</span>
              </div>
              <Separator className="bg-[#794A05]/10" />
              <div className="flex justify-between items-end pt-1">
                <span className="font-bold text-[#2a1b01]">{t('marketplace.cart.order_total')}</span>
